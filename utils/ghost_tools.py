...
from utils.ghost_ui import ChatDeleteConfirmView, CreateRepoView, DeployConfirmView, PushConfirmView
...
class TerminalConfirmView(discord.ui.View):
    ...
    @discord.ui.button(label="Execute Command", style=discord.ButtonStyle.danger, emoji="⚠️")
...
GHOST_TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "push_code",
            "description": "Push local code to GitHub. Always shows a confirmation UI before executing.",
            "parameters": {"type": "object", "properties": {}},
        },
    },
    {
...
async def execute_tool(
    ...
    bad_placeholders = [
        "none", "null", "test", "new-repo", "my-repo", "example-repo",
        "untitled", "missing", "repo-name", "undefined", "project",
    ]

    try:
        # ==========================================
        # OMNI-AGENT CAPABILITIES
        # ==========================================
        ...
        if function_name == "query_logs":
            ...
            try:
                ...
                return json.dumps({"status": "ERROR", "message": "Failed to access audit logs."})
            ...
            embed = discord.Embed(
                ...
                description="I have determined that running this shell command is necessary. Do you authorize this action, sir?",
        ...
        # ==========================================
            ...
            try:
                ...
                return json.dumps({"status": "ERROR", "message": "Database encountered an error."})
            ...
            await message.channel.send(
                embed=embed,
                view=CreateRepoView(github_client, repo_name, message.author.id, is_private),
            )
            return json.dumps({"status": "PENDING", "message": "UI spawned."})

        elif function_name == "delete_repository":
            raw_name = args.get("name", "")
            repo_name = raw_name.lower().replace(" repo", "").replace(" repository", "").strip()

            if not repo_name:
                await message.channel.send(
                    "🛑 **Missing Parameters.** Sir, please specify the exact repository name you wish to delete."
                )
                return json.dumps({"status": "ABORTED", "message": "Missing repository name."})

            try:
                # FIX: Correctly offload both get_user() and get_repo() to thread
                def _get_repo():
                    user = github_client.get_user()
                    return user.get_repo(repo_name)

                repo = await asyncio.to_thread(_get_repo)
            except Exception:
                await message.channel.send(
            ...
            try:
                def _list_repos():
                    user = github_client.get_user()
                    return list(user.get_repos(sort="updated", direction="desc")[:10])

                repos = await asyncio.to_thread(_list_repos)

                if not repos:
                    return json.dumps({"status": "SUCCESS", "message": "Inform user no repositories were found."})
                ...
                return json.dumps({"status": "ERROR", "message": "GitHub API rejected request."})
            ...
            # FIX: Wrap subprocess in asyncio.to_thread and spawn PushConfirmView
            try:
                try:
                    ...
                    return json.dumps({"status": "ERROR", "message": "Not a valid git repository."})

                # Generate diff and commit message off the event loop
                def _get_diff():
                    try:
                        return subprocess.check_output(
                            "git diff", shell=True, stderr=subprocess.PIPE
                        ).decode().strip() or "New untracked files"
                    except subprocess.CalledProcessError:
                        return "New untracked files"
                ...
                comp = await groq_client.chat.completions.create(
                    messages=[
                        {
                            "role": "system",
                            "content": "Generate a single-line git commit message. No markdown. No quotes.",
                        },
                        {"role": "user", "content": diff[:2000]},
                    ],
                    model="llama-3.3-70b-versatile",
                )
                commit_msg = comp.choices[0].message.content.strip().replace('"', "").replace("'", "")

                # FIX: Spawn PushConfirmView — never push without user approval
                embed = discord.Embed(
                    title="📤 Push Authorization Required",
                    description="I have staged the changes and prepared a commit message. Authorize the push to origin?",
                    color=discord.Color.blurple(),
                )
                embed.add_field(
                    name="💬 Commit Message", value=f"`{commit_msg}`", inline=False
                )
                embed.add_field(
                    name="⚠️ Warning",
                    value="This will run `git add . → git commit → git push`. Ensure you're on the correct branch.",
                    inline=False,
                )
                await message.channel.send(
                    embed=embed,
                    view=PushConfirmView(commit_msg, message.author.id),
                )
                return json.dumps({
                    "status": "PENDING",
                    "message": "Push UI spawned. Awaiting user authorization. Do not narrate until user confirms.",
                })

            except Exception:
                logger.error(f"Push Code Error: {traceback.format_exc()}")
                return json.dumps({"status": "ERROR", "message": "Failed to prepare push sequence."})

        elif function_name == "check_workflow":
            repo_name = args.get("repo_name")
            if not repo_name:
                return json.dumps({"status": "ERROR", "message": "Request repository name."})
            try:
                # FIX: Both get_user() and get_repo() offloaded together
                def _get_runs():
                    user = github_client.get_user()
                    repo = user.get_repo(repo_name)
                    return list(repo.get_workflow_runs()[:3])

                runs = await asyncio.to_thread(_get_runs)
                run_data = [
            ...
            if action == "on" and not username:
                return json.dumps({"status": "ERROR", "message": "Request GitHub username."})
            ...
            try:
                ...
                embed = discord.Embed(
                    title="📊 System Health",
                    color=(
                        discord.Color.green() if cpu < 90 and ram < 90 else discord.Color.red()
                    ),
                )
            ...
            if (
                ...
                return json.dumps({"status": "ABORTED", "message": "Invalid repo name."})
            if len(content.strip()) < 5:
                await message.channel.send("🛑 **Missing Payload.** Describe what to build.")
            ...
            await message.channel.send(
                ...
                view=DeployConfirmView(
                    github_client, groq_client, repo_name, content,
                    message.author.id, is_private,
                ),
            )
            return json.dumps({"status": "PENDING", "message": "Deployment UI Spawned."})

        elif function_name == "add_to_backlog":
            task = args.get("task_description")
            if not task or len(task.strip()) < 5:
                return json.dumps({"status": "ERROR", "message": "Insufficient description."})
            try:
                def _add_task():
                    with sqlite3.connect("data/dev_stats.db") as conn:
                        conn.cursor().execute(
            ...
            if results is None:
                return json.dumps({"status": "ERROR", "message": "Search engine failed."})
            if not results:
            ...
            try:
                ...
                embed.add_field(name="🔋 Power Level", value=f"`{battery_pct}%`", inline=True)
                embed.add_field(name="💬 Est. Messages Left", value=f"`~{est_msgs:,}`", inline=True)
                embed.add_field(
                ...
                return json.dumps({"status": "ERROR", "message": "Failed to read diagnostic sensors."})
